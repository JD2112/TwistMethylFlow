  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        % Balanced, elegant heading hierarchy with tightened spacing
        \setkomafont{section}{\color{milouslate}\Large\bfseries}
        \setkomafont{subsection}{\color{milouteal}\large\bfseries}
        \setkomafont{subsubsection}{\color{milouslate}\normalsize\bfseries}
        \RedeclareSectionCommand[beforeskip=-1.5ex plus -0.2ex minus -0.2ex, afterskip=0.6ex plus 0.1ex]{section}
        \RedeclareSectionCommand[beforeskip=-1.2ex plus -0.2ex minus -0.2ex, afterskip=0.4ex plus 0.1ex]{subsection}
        \RedeclareSectionCommand[beforeskip=-0.8ex plus -0.1ex minus -0.1ex, afterskip=0.3ex plus 0.1ex]{subsubsection}
...
# Gracefully try to load DT if missing
  ...
  if (knitr::is_html_output()) {
    ...
    if (exists("has_dt") && has_dt) {
      dt_tbl <- DT::datatable(df, 
                           ...
                           rownames = FALSE)
      if (!is.null(note)) {
        return(tagList(dt_tbl, tags$p(class = "table-note", tags$em(note))))
      }
      return(dt_tbl)
    } else {
